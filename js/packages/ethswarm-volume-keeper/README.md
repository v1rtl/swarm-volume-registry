...
Not a registry SDK. The export surface is the cycle and the types it returns — the reads, the ABI and the send loop behind it are internal, so there is no second way to drive the registry that has to be kept honest against the contract. Deployment concerns are yours too: no transports, chain table, RPC list, key handling or env parsing in here. Two reference bots supply them: [`js/workers/gas-boy`](../../workers/gas-boy) (Cloudflare Worker) and [`services/keeper`](../../../services/keeper) (GitHub Actions cron).
